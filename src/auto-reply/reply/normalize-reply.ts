...
export function normalizeReplyPayload(
  ...
  // AND propagate the parsed directives to the payload.
  let replyDirectives: Partial<ReplyPayload> = {};
  if (text) {
    ...
    if (parsed.replyToCurrent) {
      replyDirectives.replyToCurrent = true;
      // If we have a replyToCurrent tag, we generally want to ensure replyToId is cleared
      // unless it was explicitly provided in the tag (which parseInlineDirectives handles via replyToId).
    }
    if (parsed.replyToId) {
      replyDirectives.replyToId = parsed.replyToId;
    }
    if (parsed.audioAsVoice) {
      replyDirectives.audioAsVoice = true;
    }
  }
  ...
  return { ...enrichedPayload, ...replyDirectives, text };
}
