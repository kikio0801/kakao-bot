...
export function stripEnvelope(text: string): string {
  const directiveStripped = parseInlineDirectives(text, {
    stripAudioTag: true,
    stripReplyTags: true,
  }).text;

  const match = directiveStripped.match(ENVELOPE_PREFIX);
  if (!match) {
    return directiveStripped;
  }
  const header = match[1] ?? "";
  if (!looksLikeEnvelopeHeader(header)) {
    return directiveStripped;
  }
  return directiveStripped.slice(match[0].length);
}
