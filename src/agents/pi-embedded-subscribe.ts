...
export function subscribeEmbeddedPiSession(params: SubscribeEmbeddedPiSessionParams) {
  ...
  const emitBlockChunk = (text: string) => {
    ...
    const blockStripped = stripDowngradedToolCallText(stripBlockTags(text, state.blockState));
    // Proactively strip reply tags from the chunk to avoid leaking them to the stream
    // before the replyDirectiveAccumulator consumes them.
    const chunk = parseInlineDirectives(blockStripped, {
      stripAudioTag: false,
      stripReplyTags: true,
    }).text.trimEnd();
    if (!chunk) {
