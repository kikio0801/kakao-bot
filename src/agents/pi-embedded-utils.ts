...
export function extractAssistantText(msg: AssistantMessage): string {
  ...
  const blocks = Array.isArray(msg.content)
    ? msg.content
        .filter(isTextBlock)
        .map((c: { type: "text"; text: string }) => {
          const reasoningStripped = stripThinkingTagsFromText(
            stripDowngradedToolCallText(stripMinimaxToolCallXml(c.text)),
          );
          // Strip reply tags that might be embedded or leak into text content.
          return parseInlineDirectives(reasoningStripped, {
            stripAudioTag: false,
            stripReplyTags: true,
          }).text.trim();
        })
...
export function promoteThinkingTagsToBlocks(message: AssistantMessage): void {
  ...
  const hasThinkingBlock = message.content.some((block: any) => block.type === "thinking");
  if (hasThinkingBlock) {
    return;
  }
